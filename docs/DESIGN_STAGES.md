...
**Primary audio rule**: Episode identification uses the shared audio-selection
policy before invoking the shared transcription service.
...
6. Subtitle generation derives a **display subtitle** from the canonical JSON by
   ...
   transcript content.
7. **Formatter failure is explicit**: subtitle formatting failure marks that
   episode subtitle job failed.
8. **Duration source**: subtitle filtering/validation use actual encoded-media
...
- **Canonical vs display artifacts**:
  - canonical WhisperX SRT/JSON artifacts live in the current stage work area
...
### 6.11 Resume and Failure Isolation
