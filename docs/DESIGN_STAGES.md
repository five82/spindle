...
**Primary audio rule**: Episode identification uses the shared audio-selection policy before WhisperX transcription. Transcription is generated fresh for each stage input; Spindle does not persist WhisperX transcripts across runs or stages because episode-level cache identities can become stale when title selection changes.
...
6. Spindle does not persist canonical WhisperX artifacts in a shared cache.
   Episode identification, commentary detection, and subtitling each generate
   transcripts from their current stage input so stale transcripts cannot be
   reused after title-selection or asset-routing changes.
...
9. **Duration source**: subtitle filtering/validation use actual encoded-media
...
transcript data** before display formatting. Canonical WhisperX artifacts in the
stage work area remain unchanged.
...
- **Canonical vs display artifacts**:
  - canonical WhisperX SRT/JSON artifacts live only in the current stage work
    area and are not persisted in a shared cache
...
(see DESIGN_INFRASTRUCTURE.md Section 9) to invoke WhisperX. The `whisperxSem`
semaphore is held by the subtitling stage for the duration of transcription
work.
