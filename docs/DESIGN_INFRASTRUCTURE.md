...
Stage notifications include concurrent pipeline context when multiple items
are actively processing. Each notification message is appended with an
"Also processing" line listing other in-progress items and their stages.

The `rip_complete` notification additionally includes a "Drive is available
for next disc." line, signaling that a new disc can be inserted while the
completed item continues through later stages.
...
which queries all in-progress items and excludes the item being notified
about. Returns empty string when no other items are active.
