...
Notifications are enabled when `ntfy_topic` is non-empty. There are no
per-event config gates; emission is governed only by the canonical notification
policy described below.

### 2.2 Pipeline Context in Notifications

User-facing notifications may include concurrent pipeline context when multiple
items are actively processing. Each message may append an `Also processing`
line listing other in-progress items and their humanized stages.

The `rip_complete` and `rip_cache_hit` notifications additionally include a
`Drive is available for next disc.` line, signaling that a new disc can be
inserted while the completed item continues through later stages.

Example `rip_complete` message:
```
Ripped Avatar (2 titles)
Drive is available for next disc.
Also processing: Breaking Bad (encoding)
```

The context is generated by `queue.FormatAlsoProcessing(store, excludeID)`.
It excludes the item being notified about, caps the visible list, and appends
`+N more` when additional items are active.

### 2.3 Event Types

Notifications are intentionally biased toward clear outcomes over stage-start
chatter. The canonical event set is:

| Event                      | Priority | Tags            | Purpose |
|---------------------------|----------|-----------------|---------|
| `item_queued`             | default  | queue           | Item accepted for processing |
| `identification_complete` | default  | identify        | Identification finished |
| `rip_cache_hit`           | low      | rip,cache       | Rip restored from cache |
| `rip_complete`            | default  | rip             | Rip finished; drive available |
| `encode_complete`         | default  | encode          | Encoding finished |
| `review_required`         | high     | review,warning  | Terminal completed-with-problems outcome |
| `pipeline_complete`       | default  | complete        | Terminal clean success to library |
| `queue_started`           | default  | queue           | Backlog cycle started |
| `queue_completed`         | default  | queue           | Matching backlog cycle drained |
| `error`                   | high     | error           | Terminal failure |
| `test`                    | low      | test            | Manual test send |

Behavior rules:
- No notification per stage start.
- No notification per individual review reason.
- Validation and quality issues accumulate into the terminal
  `review_required` outcome.
- `queue_started` is sent only when the queue transitions into a meaningful
  backlog (at least 2 active items).
- `queue_completed` is sent only when a matching `queue_started` was emitted
  for that backlog cycle.
- `review_required` is the canonical terminal problem notification for full
  review routing, partial library/review routing, and other completed items
