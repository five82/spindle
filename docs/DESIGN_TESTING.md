...
| `Qwen3-ASRRunner` | `transcription` | `Run(ctx, input, opts) (*RawOutput, error)` | `transcription` |
...
| `transcription.Service` | Stub | Return canned canonical Qwen3-ASR artifacts (SRT + JSON) |
...
4. **Edge cases in external tool parsing**: MakeMKV robot format variations,
   ffprobe output quirks, canonical transcript SRT/JSON output formats, subtitle formatter invocation.
...
- Prefer golden fixtures for real-world bad wrapping / subtitle artifact cases.
