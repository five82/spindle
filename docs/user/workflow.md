...
- `episode_identification` *(TV only)* - Parakeet + OpenSubtitles correlate ripped files to definitive episode numbers
...
- `subtitling` *(optional)* - Parakeet transcription generates subtitle sidecars; forced subtitles optionally fetched from OpenSubtitles
...
1. For TV shows with OpenSubtitles enabled, Spindle compares transcription artifacts against OpenSubtitles references to map ripped files to definitive episode numbers.
...
2. Uses transcription artifacts and LLM classification to identify commentary vs. primary audio tracks.
...
When `subtitles.enabled = true`, Spindle generates subtitles from the actual audio using Parakeet transcription. Subtitles are generated per encoded asset.

1. Spindle extracts the primary audio track.
2. **Parakeet transcription**: generates canonical transcript artifacts (raw SRT + JSON output).
3. **Subtitle formatting**: the subtitle stage filters transcription hallucination artifacts from derived working transcript data, then uses Stable-TS regrouping/formatting to produce the final viewer-facing SRT.
...
5. **Forced subtitles** (optional): when OpenSubtitles is configured and a forced subtitle track is detected, foreign-parts-only subtitles are fetched from OpenSubtitles and aligned against the transcription output via text-based matching.
