...
- **Logs**: `<log_dir>/` stores `spindle-*.log` (one per daemon start, DEBUG-level JSON), `spindle.log` symlink to the latest run (when available), and the queue database (`queue.db`). Log retention is controlled by `[logging].retention_days` in `config.toml` (default 60; set 0 to disable pruning).
