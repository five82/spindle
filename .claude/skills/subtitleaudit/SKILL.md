---
name: subtitleaudit
description: Audit and fix ASR transcription errors in MKV subtitle tracks. Use /subtitleaudit <path_to.mkv> to review and correct the primary embedded subtitle.
user-invocable: true
...
Review and correct ASR transcription errors in MKV-embedded subtitles.
...
This skill extracts the primary (non-forced) SRT subtitle from an MKV file, reviews it for obvious ASR transcription errors, presents proposed corrections for user approval, applies the approved edits, and muxes the corrected subtitle back into the MKV.

It is intended for subtitles generated by Spindle's transcription pipeline (currently Qwen3-ASR + Qwen3-ForcedAligner), but the audit process is generic and applies to any machine-generated subtitle track.
...
Analyze the file for **obvious** ASR transcription errors. Err heavily on the side of caution -- false positives (incorrect "corrections") are worse than missed errors.

**DO flag these (high confidence):**

| Error Type | Description | Example |
|------------|-------------|---------|
| Residual non-speech artifacts | The same short cue or phrase appearing 3+ times throughout the file in isolation -- not part of a conversation, often surrounded by large timestamp gaps (30s+) on both sides. Look for repeated junk text that behaves like model noise rather than dialogue. | The same isolated two-word junk cue appearing 11 times scattered across the file, each time with no surrounding dialogue |
...
| Skip | Why |
|------|-----|
| Proper noun spelling | The ASR may have the correct uncommon spelling; we can't verify without the script |
...
**Batching:** Consecutive cues with the same error type (e.g., a run of credits lyrics or a cluster of repeated artifact cues) should be batched into a single numbered item listing the cue range. This keeps the report readable.
