...
class DraptoEncoder:
    ...
    def _build_drapto_command(self, input_file: Path, output_dir: Path) -> list[str]:
        ...
        cmd = [
            ...
            "--progress-json",  # Enable structured JSON progress output (automatically runs in foreground)
        ]
...
class DraptoService:
    """Consolidated Drapto service combining wrapper and implementation."""
    ...
    def encode_file(
        ...
        try:
            ...
            # Perform encoding using output_dir (drapto expects directory, not file)
            output_dir = output_file.parent
            result = self.encoder.encode_file(
                input_file=input_file,
                output_dir=output_dir,
                progress_callback=lambda data: (
                    progress_callback(
                        data.get("message", "Encoding..."),
    ...
    def validate_drapto_available(self) -> bool:
        """Check if drapto is available and working."""
        return self.encoder.check_drapto_availability()
    ...
    def get_encoder_info(self) -> dict[str, Any]:
        ...
        return {
            ...
            "config": {
                "quality_hd": self.config.drapto_quality_hd,
                "quality_sd": self.config.drapto_quality_sd,
                "quality_uhd": self.config.drapto_quality_uhd,
...
__all__ = ["DraptoEncoder", "DraptoService", "EncodeResult"]
