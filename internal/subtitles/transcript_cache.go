...
func (g *Generator) tryReuseCachedTranscript(target subtitleTarget, env *ripspec.Envelope) (GenerateResult, bool) {
	...
	if err != nil || info.Size() == 0 {
		if g.logger != nil {
			g.logger.Info("contentid transcript cache miss",
	...
	if err != nil || cues == 0 {
		if g.logger != nil {
			g.logger.Warn("contentid transcript cache rejected",
	...
	if err := os.MkdirAll(filepath.Dir(destPath), 0o755); err != nil {
		if g.logger != nil {
			g.logger.Warn("contentid transcript cache dir creation failed",
	...
	if err := fileutil.CopyFile(cachedPath, destPath); err != nil {
		if g.logger != nil {
			g.logger.Warn("contentid transcript cache copy failed",
	...
	if g.logger != nil {
		g.logger.Info("contentid transcript cache hit",
			logging.String(logging.FieldDecisionType, "transcript_cache"),
...
// the typed ContentIDTranscripts map.
func lookupTranscriptPath(env *ripspec.Envelope, episodeKey string) string {
	if env == nil || len(env.Attributes.ContentIDTranscripts) == 0 {
		return ""
	}
	...
	return strings.TrimSpace(env.Attributes.ContentIDTranscripts[key])
}
