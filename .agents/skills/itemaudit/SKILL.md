...
2. **Timing/progress anomalies** (from `logs.stages` and `logs.events`):
   ...
   - Item lifecycle: `event_type=item_complete` is the one-line completion summary (per-stage `<stage>_duration` attrs plus `total_wall_time`); `event_type=operator_action` records user-initiated retry/stop/remove/clear/disc-pause/daemon-stop; `event_type=startup_queue_state` shows what a daemon restart resumed.
   - **Drain restarts are NORMAL, not anomalies.** `spindle stop` (and therefore every deploy) drains by default: dispatch pauses, non-drive workers (encoding, GPU stages) are cancelled and revert to pending, in-flight drive work (identification, ripping) finishes, then the daemon exits and the next start resumes the queue. Expect this mid-item signature and do not flag it: `operator_action` with `action=daemon_stop` (`mode=drain` or `mode=force`), `decision_type=daemon_drain` (`draining` → per-worker `cancelled` → `drained`), `startup_queue_state` on the next start, logs spanning multiple daemon log files, and a SECOND `stage started`/`stage_start` pair for each resumed stage.
   - Resume evidence after a drain/restart is also normal: a repeated `encode_start` for the same episode with reel resuming from completed chunks (`encoding_progress` starting at nonzero `chunks_complete`); `decision_type=staging_cleanup` reporting `kept_ripped_titles` and `decision_type=title_rip` with `decision_result=skipped` for titles preserved across the restart; `event_type=makemkv_rip_cancelled` when a force-stop killed a rip. Timing caveat: a resumed stage's `stage_duration`/task timing covers only the final run, while `total_wall_time` includes time the daemon was stopped — do not flag that mismatch as a hang.
...
6. **Parse encoding details** from `encoding.snapshot`:
   ...
   - Target-quality search outcome: `envelope.attributes.encode_stats[].target_quality.metrics[]` carries per-encode `score_min/mean/max`, `probes_per_chunk`, and `stop_reasons` (with debug logs, the DEBUG `reel verbose` lines `TQ summary` / `TQ decisions` / `TQ probe` / `TQ final` give the per-chunk detail). Read `stop_reasons` by name:
...
| Interrupted work marked failed | Any | `item.error_message` with `signal: killed` / `context canceled` coinciding with a stop, drain, or daemon restart | Cancellation-classification bug: a stop/drain must revert tasks to pending, never fail the item |
| Encode resume state discarded without cause | Encoding | Reel WARN "discarded stale resume state" with no reel upgrade, settings change, or rip re-run to explain it | Encode restarted from chunk 0; wasted wall time — investigate what invalidated the resume manifest |
